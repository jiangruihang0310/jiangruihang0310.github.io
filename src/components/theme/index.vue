		...
		<span class="toggle__content">
			...
			<span aria-hidden=true class="pilot__container">
				<span class="pilot-bear">
					<img :src="require('@/assets/xx.jpg')" alt="pilot-bear" class="pilot" />
				</span>
			</span>
			<!-- light模式的云1 -->
			<svg aria-hidden=true class="toggle__backdrop" xmlns="http://www.w3.org/2000/svg" fill="none"
	...
	setup() {
		onMounted(() => {
			...
			const TOGGLE = () => {
				...
				localStorage.setItem('isDark', IS_PRESSED)
...
.toggle {
	...
	background: hsl(calc(204 + (var(--dark, 0) * 25)),
...
.toggle__star:before {
	...
	width: 356%;
	background:
		radial-gradient(rgba(255, 255, 255, 0.25) 40%, transparent 40.5%),
		radial-gradient(rgba(255, 255, 255, 0.25) 56%, transparent 56.5%) rgba(255, 255, 255, 0.25);
	border-radius: 50%;
	aspect-ratio: 1;
	position: absolute;
	top: 50%;
